class BuyingItemsController < ApplicationController
  ...
  def buying_item_params
    params.require(:buying_item_shipping).permit(:address_number,:prefecture_id,:municipality,:address,:building_name,:phone_number,:buying_item_id).merge(user_id: current_user.id,item_id: params[:item_id])
  end
